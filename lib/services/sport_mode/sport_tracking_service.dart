...
class SportTrackingService {
  ...
  static const int MIN_SAMPLES_FOR_CALIBRATION =
      10; // Need at least 10 to calibrate
  ...
  int get calibrationProgress =>
      (_uprightSamples.length * 100 / MIN_SAMPLES_FOR_CALIBRATION)
          .clamp(0, 100)
          .toInt();
  ...
  void _checkAndUpdateAutoCalibration() {
    ...
    bool isMovingSlowly =
        _currentSpeed >= UPRIGHT_SPEED_MIN &&
        _currentSpeed <= UPRIGHT_SPEED_MAX;

    bool isStable =
        _rawGyroX.abs() < STABILITY_THRESHOLD &&
    ...
    if ((isMovingSlowly || _currentSpeed == 0) && isStable && lowAcceleration) {
      ...
      onCalibrationMessage?.call(
        'Hold steady... ${_stableFrameCount}/${STABLE_FRAMES_REQUIRED}',
      );
  ...
  void _checkForPositionChange() {
    ...
    print(
      '[DEBUG] currentRef=(${currentRef.x.toStringAsFixed(3)}, ${currentRef.y.toStringAsFixed(3)}, ${currentRef.z.toStringAsFixed(3)})',
    );
    print(
      '[DEBUG] newRef=(${newRef.x.toStringAsFixed(3)}, ${newRef.y.toStringAsFixed(3)}, ${newRef.z.toStringAsFixed(3)})',
    );
    ...
    if (currentMagG > 0.1 && newMagG > 0.1) {
      final cosAngleG = (dotProductG / (currentMagG * newMagG)).clamp(
        -1.0,
        1.0,
      );
      ...
      print(
        '[DEBUG] Gravity angle difference = ${angleDifferenceG.toStringAsFixed(2)}°',
      );

      // If tilt changed significantly (e.g., phone slipped)
      if (angleDifferenceG > POSITION_CHANGE_ANGLE_THRESHOLD) {
        onCalibrationMessage?.call(
          'Major tilt change detected. Recalibrating...',
        );
        shouldRecalibrate = true;
      }
    } else {
      // Gravity magnitude too small for reliable tilt check
    }
    ...
    if (!shouldRecalibrate) {
      ...
      print(
        '[DEBUG] oldForward=(${oldForward.x.toStringAsFixed(3)}, ${oldForward.y.toStringAsFixed(3)}, ${oldForward.z.toStringAsFixed(3)})',
      );
      print(
        '[DEBUG] newForward=(${newForward.x.toStringAsFixed(3)}, ${newForward.y.toStringAsFixed(3)}, ${newForward.z.toStringAsFixed(3)})',
      );
      ...
      if (oldMagF > 0.01 && newMagF > 0.01) {
        ...
        print(
          '[DEBUG] Forward angle difference = ${angleDifferenceF.toStringAsFixed(2)}°',
        );

        // If the forward direction changed significantly (e.g., 90/180 degree flip)
        if (angleDifferenceF > POSITION_CHANGE_ANGLE_THRESHOLD) {
          onCalibrationMessage?.call(
            'Phone position/flip detected. Recalibrating...',
          );
          shouldRecalibrate = true;
        }
      } else {
        // Forward magnitude too small for reliable forward check
      }
    }
    ...
    if (shouldRecalibrate) {
      ...
      _performAutoCalibration(
        newRef,
      ); // Use the new gravity vector for recalibration
      ...
      if (_isCalibrated &&
          _calibrationStatus != AutoCalibrationStatus.calibrated) {
  ...
  double _calculateLeanAngleWithCalibration(
    double accelX,
    double accelY,
    double accelZ,
  ) {
    ...
    final referenceGravity = _Vector3(
      _calibrationRefX,
      _calibrationRefY,
      _calibrationRefZ,
    );
    ...
    final double cosAngle = (dotProduct / (currentMag * refMag)).clamp(
      -1.0,
      1.0,
    );
